...
from data_loader import SYSUData, RegDBData, TestData,TestDataAdd
...
parser.add_argument('--resume', '-r', default='llcm_djan_p4_n24_lr_0.1_seed_0_best.t', type=str,
                    help='resume from checkpoint')
parser.add_argument('--model_path', default='result/DJANNet/save_model/',
...
if dataset == 'sysu':
    ...
    n_class = 206
    test_mode = [1, 2]  # [1, 2]: visible to thermal; [2, 1]: thermal to visible;
elif dataset == 'llcm':
    data_path = '/dataset/LLCM/'
    n_class = 713
    test_mode = [2, 1]  # [1, 2]: visible to thermal; [2, 1]: thermal to visible;
...
if dataset == 'sysu':
    ...
    if len(args.resume) > 0:
        ...
        if os.path.isfile(model_path):
            ...
            checkpoint = torch.load(model_path, map_location={
                'cuda:3': 'cuda:0','cuda:2': 'cuda:0', 'cuda:1': 'cuda:0'})
    ...
    for trial in range(10):
        ...
        cmc_pool, mAP_pool, mINP_pool = eval_sysu(
            -distmat_pool, query_label, gall_label, query_cam, gall_cam,args.mode)

        # fc feature
        distmat = np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
        cmc, mAP, mINP = eval_sysu(-distmat, query_label,
                                   gall_label, query_cam, gall_cam,args.mode)
        if trial == 0:
            all_cmc = cmc
            all_mAP = mAP
            all_mINP = mINP
    ...
    for trial in range(10):
        test_trial = trial + 1
        model_path = checkpoint_path + 'regdb_djan_p4_n24_lr_0.1_seed_0_trial_{}_best.t'.format(test_trial)
        if os.path.isfile(model_path):
            print('==> loading checkpoint {}'.format(args.resume))
            checkpoint = torch.load(model_path, map_location={
        ...
        query_img, query_label = process_test_regdb(
            data_path, trial=test_trial, modal=test_mode[0])
        gall_img, gall_label = process_test_regdb(
            data_path, trial=test_trial, modal=test_mode[1])
        ...
        if args.tvsearch:
            ...
            cmc_pool, mAP_pool, mINP_pool = eval_regdb(
                -distmat_pool, gall_label, query_label, mode = test_mode[0])

            # fc feature
            distmat = np.matmul(gall_feat_fc, np.transpose(query_feat_fc))
            cmc, mAP, mINP = eval_regdb(-distmat, gall_label, query_label, mode = test_mode[0])
        else:
            # pool5 feature
            distmat_pool = np.matmul(
                query_feat_pool, np.transpose(gall_feat_pool))
            cmc_pool, mAP_pool, mINP_pool = eval_regdb(
                -distmat_pool, query_label, gall_label, mode = test_mode[0])

            # fc feature
            distmat = np.matmul(query_feat_fc, np.transpose(gall_feat_fc))
            cmc, mAP, mINP = eval_regdb(-distmat, query_label, gall_label, mode = test_mode[0])
    ...
    for trial in range(10):
        ...
        if 1:
            ...
            cmc, mAP, mINP = eval_llcm(-distmat, query_label, gall_label, query_cam, gall_cam,mode=test_mode[0])
            cmc_pool, mAP_pool, mINP_pool = eval_llcm(-distmat_A, query_label, gall_label, query_cam, gall_cam,mode=test_mode[0])
