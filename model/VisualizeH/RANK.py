...
def eval_vision_H(distmat, q_pids, g_pids, queryset, gallset, trial, max_rank=20):
    H = 392
    W = 196
    ...
    tmp = q_pids[:, np.newaxis]
    matches = np.where(g_pids[indices] == q_pids[:, np.newaxis])
    ...
    for i in range(0, 3803, 30):
        ...
        for row in range(fig_per_num):
            ...
            for col in range(1 + 1, rank + 1 + 1):
                ...
                axs[col].axis('off')
        if not os.path.isdir('images/' + str(begin) + '_' + str(end)):
            os.makedirs('images/ranking/' + str(begin) + '_' + str(end))

        output_dir = 'images/ranking/' + str(begin) + '_' + str(end) + '/tmp' + str(trial) + '.png'
