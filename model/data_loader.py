...
from utils import SYSU_LABEL2PID, SYSU_Refer, RegDB_Refer, LLCM_Refer
# import open_clip
...
class TestDataAdd(data.Dataset):
    def __init__(self, test_img_file, test_label, transform=None, img_size=(144, 288)):
        test_image = []
        for i in range(len(test_img_file)):
            img = Image.open(test_img_file[i])
    ...
    def __getitem__(self, index):
        ...
        text_token = self.tokenizer(text).squeeze(0)

        return img1, target1, text_token
