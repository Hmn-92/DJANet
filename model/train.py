...
parser.add_argument('--model_path', default='result/DJANNet/save_model_all/', type=str, help='model save path')
parser.add_argument('--log_path', default='result/DJANNet/log/', type=str, help='log save path')
parser.add_argument('--vis_log_path', default='result/MDJANNet/log/vis_log/', type=str, help='log save path')
...
parser.add_argument('--gpu', default='2', type=int, help='gpu device ids for CUDA_VISIBLE_DEVICES')
...
if dataset == 'sysu':
    data_path = '/dataset/SYSU-MM01_generate_2/'
...
suffix = dataset
suffix = suffix + '_djan_p{}_n{}_lr_{}_seed_{}'.format(
...
def train(epoch):
    ...
    for batch_idx, (input10, input11, input20, input21, label1, label2) in enumerate(trainloader):
        ...
        with amp.autocast(enabled=True):
            ...
            feat, out0,loss_cll = net(input10, input11, input20, input21)
            ...
            loss = loss_id + loss_tri + loss_cll
...
def test(epoch):
    ...
    # evaluation
    if dataset == 'regdb':
        cmc, mAP, mINP = eval_regdb(-distmat, query_label, gall_label, mode=3)
        cmc_att, mAP_att, mINP_att = eval_regdb(
            -distmat_att, query_label, gall_label, mode=3)
    elif dataset == 'sysu':
        cmc, mAP, mINP = eval_sysu(-distmat, query_label,
                                   gall_label, query_cam, gall_cam)
        cmc_att, mAP_att, mINP_att = eval_sysu(
            -distmat_att, query_label, gall_label, query_cam, gall_cam, mode=3)
    elif dataset == 'llcm':
        cmc, mAP, mINP = eval_llcm(-distmat, query_label,
                                   gall_label, query_cam, gall_cam, mode=3)
